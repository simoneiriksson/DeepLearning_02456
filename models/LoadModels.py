...
def LoadVAEmodel(folder, model_type=None, device="cpu", ):
    parameters = torch.load(folder + "parameters.pt", map_location=torch.device(device))
    ...
    if (model_type == None) or model_type == "Cyto":
        vae = CytoVariationalAutoencoder(parameters['image_shape'], parameters['latent_features'])
    if model_type == 'Cyto_nonvar':
        vae = CytoVariationalAutoencoder_nonvar(parameters['image_shape'], parameters['latent_features'])
        vi = VariationalInference_nonvar(beta=parameters['beta'])
    if model_type == 'basic':
        vae = VariationalAutoencoder(parameters['image_shape'], parameters['latent_features'])
    if model_type == 'Conv_simon':
        vae = ConvVariationalAutoencoder(parameters['image_shape'], parameters['latent_features'])
    if model_type == 'SparseVAE':
        vae = SparseVariationalAutoencoder(parameters['image_shape'], parameters['latent_features'])
    
    vae.load_state_dict(torch.load(folder + "vae_parameters.pt", map_location=torch.device(device)))
    return vae, validation_data, training_data, parameters, vi


def initVAEmodel(parameters):
    ...
    return vae, validation_performance, training_performance, parameters, vi